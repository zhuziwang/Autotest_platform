...
from Product.models import KeywordPackage, KeywordClazz, KeywordMethod
...
class Environment:
    ...
    def get(request, environment_id):
        """
        model_to_dict: 返回第一个参数的实例对象，第一个参数是实例对象，第二个参数是指定字段，第三个参数是排除字段
        :return: 返回environment表中该id字段的指定字段内容
        """
        e = get_model(environment, id=environment_id)
        if not e:
...
class Keyword:
    ...
    # @staticmethod
    # @post
    # def keyword_package_clazz_method(request):
    @staticmethod
    @post
    def create(request):
        try:
    ...
    @staticmethod
    def kwpackage(request):
        package = get_request_body(request)
        kwp = KeywordPackage.objects.all()
        total = len(kwp)
        kwp_list = list()
        for kwp in kwp:
            kwp_dict = model_to_dict(kwp, ['id', 'package'])
            kwp_list.append(kwp_dict)
        keyword_package = dict()
        keyword_package["total"] = total
        keyword_package["package"] = kwp_list
        return JsonResponse.OK(message='ok', data=keyword_package)

    @staticmethod
    def kwpclazz(request, package_id):
        kwc = KeywordClazz.objects.filter(package=package_id)
        total = len(kwc)
        kwc_list = list()
        for kwc in kwc:
            kwc_dict = model_to_dict(kwc, ['id', 'clazz'])
            kwc_list.append(kwc_dict)
        keyword_clazz = dict()
        keyword_clazz["total"] = total
        keyword_clazz["clazz"] = kwc_list
        return JsonResponse.OK(message='OK', data=keyword_clazz)

    @staticmethod
    def kwpclazz_all(request):
        kwc = KeywordClazz.objects.all()
        total = len(kwc)
        kwc_list = list()
        for kwc in kwc:
            kwc_dict = model_to_dict(kwc, ['id', 'clazz'])
    ...
    @staticmethod
    def kwpmethod(request, clazz_id):
        kwm = KeywordMethod.objects.filter(clazz=clazz_id)
        total = len(kwm)
        kwm_list = list()
        for kwm in kwm:
            kwm_dict = model_to_dict(kwm, ['id', 'method'])
            kwm_list.append(kwm_dict)
        keyword_method = dict()
        keyword_method["total"] = total
        keyword_method["method"] = kwm_list
        return JsonResponse.OK(message='Ok', data=keyword_method)

    @staticmethod
    def kwpmethod_all(request):
        kwm = KeywordMethod.objects.all()
        total = len(kwm)
        kwm_list = list()
        for kwm in kwm:
            kwm_dict = model_to_dict(kwm, ['id', 'method'])
...
class TestCase:
    ...
    def test(request, testcase_id):
        """
        运行测试用例，，发送到rabbit_mq队列名称为web/h5的队列
        :param testcase_id: testcase表的id
        :return:
        """
        tc = get_model(testcase, id=testcase_id)
        if not tc:
        ...
        # 教程地址：http://t.zoukankan.com/lowmanisbusy-p-10698189.html
        # 把r.id数据放进rabbit_mq消息队列
...
class TestResult:
    ...
    def find(request):
        ...
        now = timezone.now().strftime('%Y-%m-%d %H:%M:%S')
        end_time = parameter.get("endTime", now) if parameter.get("endTime", now) else now
        try:        # orm 筛选，教程地址: https://blog.csdn.net/Liang20200918/article/details/109810339
...
class Public:
    ...
    def data(request):
        """
        :return: 返回browser表中id,name,value,remake字段的数据
        """
...
class TestTasks:
    ...
    def test(request, task_id):
        """
        运行测试任务（多条测试用例），发送到rabbit_mq队列名称为web/h5_queue的队列
        :param task_id: task表的id
        :return:
        """
        t = get_model(Task, id=task_id)
        if not t:
        ...
        for tc in testcases:
            ...
            SplitTask.apply_async(args=(r.id, 1), queue="web/h5_queue")
        return JsonResponse.OK()
...
class Login:
    ...
    def get(request, login_id):
        ...
        steps_ = list()
        # 判断Login表的checkType字段（前置登陆-断言类型是element时），获取element表的id：Login表的chekValue
        ...
        for step in steps:
            ...
            for value in values:
                ...
                pa["value"] = value["value"]
                pa["key"] = value["key"]
                # 判断该测试步骤是否使用参数化（isParameter是否是True，True：使用参数化，False：没有使用参数化）
                if not pa["isParameter"]:
                    # 判断测试步骤中的value["type"]=="element"（填写内容的都是string，选择内容的都是element）
                    if value["type"] == "element":
                        ele = get_model(element, id=value["value"])
                        pa["pageId"] = ele.pageId
                        pa["elementName"] = ele.name
